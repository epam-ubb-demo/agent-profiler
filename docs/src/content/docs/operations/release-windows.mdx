---
title: Release Runbook — Windows
description: Sign and publish the Agent Profiler Windows NSIS installer release artefact.
---
...
5. Download the `.exe` artefact from the completed run.

## Verifying the Installer

1. Download the NSIS `.exe` installer artefact.
2. Right-click the `.exe` → **Properties** → **Digital Signatures** tab.
   - Verify the signature is valid and shows your organisation name.
3. Run the installer:
   - Confirm the UAC prompt shows the correct publisher name.
