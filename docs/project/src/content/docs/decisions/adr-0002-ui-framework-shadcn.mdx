...
description: Decision record for selecting shadcn/ui and Radix UI as the initial UI component library. Superseded by ADR-0007.
---

## Status

Superseded by [ADR-0007](/agent-profiler/project/decisions/adr-0007-ui-framework-epam-uui/)
...
The original plan specified EPAM UUI as the component library. However, a licence compatibility spike revealed that EPAM UUI's licence terms had not been confirmed as compatible with MIT distribution. Since the project owner (Sergio Sisternes) was unavailable to resolve this blocker, and F0.2 (Electron shell) depends on having a UI framework selected, an autonomous decision was required.
