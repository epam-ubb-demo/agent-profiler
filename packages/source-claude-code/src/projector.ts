/**
 * ClaudeCodeSessionProjector — implements {@link SessionProjector} for
 * the Claude Code tool by reconstructing a Session from enrichment events.
 *
 * This is the inverse operation of {@link ClaudeCodeEnrichmentSource.readEvents}.
 */
 ...
 * {@link ClaudeCodeEnrichmentSource}.
 *
 * Fields such as compactions, subagents, utilisation, and fanoutTurns are
 * returned as empty arrays since Claude Code sessions do not carry that data.
 */
export class ClaudeCodeSessionProjector implements SessionProjector {
  ...
  project(events: readonly EnrichmentEvent[]): Session {
    ...
    for (const e of userInteractionEvents) {
      const { turn, userMessage, assistantMessages: asstMsgs } = reconstructTurn(e.payload);
      turns.push(turn);
      if (userMessage !== null) {
