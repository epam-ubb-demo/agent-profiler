/**
 * Unit tests for VsCodeChatEnrichmentSource.
 */
...
function makeSource() {
  return new VsCodeChatEnrichmentSource([
...
const FIXTURE_REF = { tool: 'vscode-chat' as const, sessionId: SESSION_ID, locationHint: FIXTURE_FILE };
...
describe('VsCodeChatEnrichmentSource', () => {
  describe('tool', () => {
    it('has tool = vscode-chat', () => {
      expect(makeSource().tool).toBe('vscode-chat');
    });
  });
  ...
  describe('discoverSessions()', () => {
    it('discovers the fixture session via override list', async () => {
      ...
      expect(found?.tool).toBe('vscode-chat');
  ...
  describe('readEvents()', () => {
    ...
    it('skips metadata event when cursor covers it', async () => {
      ...
      const cursor = {
        ...
        category: 'metadata',
        lastOrdinal: 0,
        lastEventId: ':vscode-chat:metadata:0',
  ...
  describe('watch()', () => {
    ...
    it('returns a no-op watcher for a nonexistent file', () => {
      const ref = { tool: 'vscode-chat' as const, sessionId: 'ghost', locationHint: '/nonexistent/ghost.jsonl' };
      expect(() => {
