...
describe('SessionBrowser', () => {
  ...
  it('does not show data source toggle when App Insights is not configured', async () => {
    vi.mocked(mockElectronApi.settings.get).mockResolvedValue({ workspaceId: '', timeRangePreset: '7d' });
    vi.mocked(mockElectronApi.session.list).mockResolvedValue([
  ...
  it('shows data source toggle when App Insights is configured', async () => {
    vi.mocked(mockElectronApi.settings.get).mockResolvedValue({ workspaceId: 'my-ws-id', timeRangePreset: '7d' });
    vi.mocked(mockElectronApi.session.list).mockResolvedValue([
      makeSession({ id: 'session-1' }),
    ]);
    ...
    expect(screen.getByTestId('data-source-local')).toBeDefined();
    expect(screen.getByTestId('data-source-remote')).toBeDefined();
  });

  it('defaults to Local view and shows only local-adapter sessions', async () => {
    vi.mocked(mockElectronApi.settings.get).mockResolvedValue({ workspaceId: 'ws', timeRangePreset: '7d' });
    vi.mocked(mockElectronApi.session.list).mockResolvedValue([
    ...
    const cards = screen.getAllByTestId('session-card');
    expect(cards).toHaveLength(1);
    // local session is shown; remote is hidden
    expect(screen.getByTestId('session-count-badge').textContent).toContain('1');
  });

  it('shows only application-insights sessions when Remote is selected', async () => {
    vi.mocked(mockElectronApi.settings.get).mockResolvedValue({ workspaceId: 'ws', timeRangePreset: '7d' });
    vi.mocked(mockElectronApi.session.list).mockResolvedValue([
      makeSession({ id: 'local-1', adapter: 'copilot-cli' }),
      makeSession({ id: 'local-2', adapter: 'vscode-chat' }),
      makeSession({ id: 'remote-1', adapter: 'application-insights' }),
    ]);
    ...
    // Switch to Remote
    const remoteBtn = screen.getByTestId('data-source-remote');
  ...
  it('switching back to Local restores local sessions', async () => {
    vi.mocked(mockElectronApi.settings.get).mockResolvedValue({ workspaceId: 'ws', timeRangePreset: '7d' });
    vi.mocked(mockElectronApi.session.list).mockResolvedValue([
      makeSession({ id: 'local-1', adapter: 'copilot-cli' }),
      makeSession({ id: 'remote-1', adapter: 'application-insights' }),
    ...
    // Switch to Remote, then back to Local
    fireEvent.click(screen.getByTestId('data-source-remote'));
    await waitFor(() => {
      expect(screen.getByTestId('session-count-badge').textContent).toContain('1');
    });

    fireEvent.click(screen.getByTestId('data-source-local'));
    await waitFor(() => {
      // Back to 1 local session
      expect(screen.getByTestId('session-count-badge').textContent).toContain('1');
    });

    // The local session card must be present
